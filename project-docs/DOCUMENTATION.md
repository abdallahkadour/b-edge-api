...
> **74 documents on disk** — 62 in `b-edge-api/project-docs/` (including this index), 7 in `project-docs/agent-reviews/`, 5 in `b-edge-web/project-docs/`. Read `CLAUDE.md` first in any new chat. These counts are generated by `scripts/doc-facts.sh` and checked by `make docs-check`, so a new document that is never listed here now fails a check instead of quietly disappearing.
>
...
> `./scripts/check-docs.sh` fails when they drift from
> `project-docs/doc-facts.baseline`. That check exists because the block
> below stood at 33 migrations while the repository had 43 — ten migrations
> of drift, accumulated over a fortnight in which every individual commit
> was green. Nothing had failed, because nothing was checking.
>
> **Two corrections to the previous block, not just newer counts.**
...
| `agent-reviews/` (7 files) | **New, Sep 6, 2026. A six-domain panel review, written into `project-docs/agent-reviews/` rather than `docs/` — `docs/` is gitignored (`.gitignore:12`), so reviews placed there would never have been committed.** Start with `00-executive-synthesis.md`, which reconciles the panel and gives the P0–P3 plan. The headline is that the codebase is **better than the review brief assumed**: coupling is genuinely low (20 cross-domain edges across 17 domains), the Tailwind palette is semantic rather than scattered hex, all routes are lazy, initial transfer is **91 kB**, and the GIST exclusion constraint is a real concurrency guard verified under an 8-way race. Weaknesses are concentrated rather than systemic — no edge layer, a frontend with 8 tests across 38 routes about to receive two large refactors, and a product blocked on a business-registration question rather than on code. Three conflicts are reconciled explicitly: a component library does **not** fight the bundle budget (the five proposed components are extractions of code that already ships, and dark mode adds zero JS); **Redis is declined** because the two things already cached are the two things that go stale, and agents 02/04/06 independently converge on a **CDN** instead, which resolves a security risk, a scaling question and the caching debate in one purchase; and constant-time ownership responses are declined as not credible next to the missing WAF. The review also caught two self-inflicted items from the previous two days: ~106 lines of dead `validationMessage` orphaned by the `mapValidationError` consolidation (`c37ee2f`), and `money.upper()` reimplementing `strings.ToUpper` to avoid one stdlib import. Also documents what is recommended **against**, so it is not revisited as oversight: shadcn/Radix/React Aria (React-only — this is Angular 21), Material/PrimeNG, a scheduler, and indexes on 8 of the 11 unindexed FKs. |
| `B-Edge-Deployment-Runbook-v1.md` | **New, Sep 6, 2026.** How this goes to production and how it comes back after a disaster. **Postgres is self-hosted on the same box, deliberately** — managed Postgres sells backups, failover and patching rather than Postgres itself, only backups matter at this stage, and self-hosting removes an extension risk: the double-booking guard needs **`btree_gist`**, which managed providers gate. Documents the proxy step that is **not optional**: Fiber's `c.IP()` returns the socket peer, so behind Cloudflare all traffic would key to ONE rate-limit bucket of 600 requests per 5 minutes and every audit entry would record Cloudflare's IP instead of the admin's. `TRUSTED_PROXIES` is empty by default and a forged `CF-Connecting-IP` is verifiably ignored. Backups are `pg_dump -Fc`, verified before they are kept, rotated only after a good dump, with an off-box copy that exits non-zero when it fails rather than reporting success in cron. The **restore drill** is the part worth reading: checking that `pg_restore` exited 0 proves almost nothing, so it inserts a booking and then an overlapping one and requires the second to be REJECTED — a restore that quietly lost `btree_gist` would leave a database that looks complete and has silently lost the only thing preventing two customers being booked into one slot. Names what it does not give: no point-in-time recovery, so up to 24 hours of bookings are lost in a disaster, accepted at one launch artist and not beyond. |
