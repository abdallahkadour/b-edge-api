...
| **FRAUD-16** | **Book any artist at any salon's price.** Hold, `POST /bookings`, slots and waitlist each take an artist, store and service by ID. Send an artist with her own store and ANOTHER salon's cheapest service; then her own service and another salon's store. | **FOUND LIVE 2026-09-25, fixed the same day.** Both returned **201**: Rania booked at a $50 price set by another salon and filed under that salon, because `salon_id` was copied from the service. Service IDs are public, so this needed no insider knowledge. The waitlist checked nothing at all, and an unknown `store_id` answered 500. The check existed nowhere because it would have had to be remembered four times. | Service and store both in the artist's salon, and the artist linked to the store (`artist_stores`), enforced by one guard - `validateBookingParties` - on all four paths. A foreign service is indistinguishable from a missing one (same 404, code, message). Covered by `parties_test.go`, `TestArtistPlacement_*`, chaos 3.6/3.6b/4.3; all 9 realistic mutations killed. 0 of 52 bookings ever landed cross-salon. |
| **FRAUD-17** | **Per-artist pricing capability and money boundaries.** (a) A member (not the owner) calls `PUT /artists/salon/members/:colleagueArtistId/services/:serviceId` to set a colleague's price. (b) The OWNER of one salon calls the same route naming an artist who belongs to a **different** salon. (c) Price/deposit values outside the money whitelist on her own row. | (a) tests the capability gate itself — `member_services:write` is owner-only, so a member has it nowhere, not even for a colleague in her own salon. (b) tests the boundary a capability alone does not draw: having the capability for YOUR salon must not reach into another one. (c) is FRAUD-16's money-whitelist discipline (`internal/pkg/money`), re-run against the new per-artist write path rather than assumed to carry over. | **Measured live, 2026-09-26**, throwaway salons/artists, torn down after (residual 0): (a) **403 `SALON_ROLE_FORBIDDEN`** — "Only the salon owner can do this." *(A token that predates accepting the invitation reads `NO_SALON` instead, because `salon_id` is minted into the access token at issue and a join does not reissue it — re-logging in after accept is what surfaces the real capability check; see spec §5's departed-member note for the same token/DB-state distinction the other way around.)* (b) **404 `MEMBER_NOT_FOUND`** — indistinguishable from a nonexistent artist, same convention as FRAUD-16. (c) **400 `INVALID_PRICE`** for `1e3`, `NaN`, `-10.00` and `33.333` (scale); **422 `VALIDATION_ERROR`** for a deposit above the price. Nothing persisted in any rejected case. |
