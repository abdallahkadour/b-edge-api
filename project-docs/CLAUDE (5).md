...
1. Sweep other forms for the same generic-error pattern `extractApiErrorMessage` just fixed on guest-details
2. Start a Twilio account — has its own external approval wait, worth kicking off regardless of when WhatsApp actually gets flipped on
3. Block Dates / Onboarding screens
4. `early_bird_cutoff`/fee, VIP rule — parked product decisions whenever ready
5. Customer accounts — own dedicated session

---

*B-Edge · Beauty at the Edge · August 2, 2026*
