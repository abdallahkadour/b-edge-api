...
> 58 documents on disk (56 in `b-edge-api/project-docs/` including this index + 2 in `b-edge-web/project-docs/`). Read `CLAUDE-v6.md` first in any new chat — it supersedes all earlier CLAUDE.md versions.
>
...
> **Verified against code 2026-09-05:** **33 migrations**, **29 tables**,
> **17 domains**, **115 route registrations** (102 paths in swagger),
> **645 Go tests**. Frontend: **8 tests**, and the `ng test` targets run at
> all for the first time — all three were broken scaffolding until
> 2026-09-05. Any schema-version or domain-count figure written above
> this line is a snapshot from the day it was written, not current. The
> current engineering context is `CLAUDE.md`.
...
|---|---|---|
| **Billing tests** | `internal/billing` went from 0 to 44 service-layer tests (83 across the whole billing surface as of 2026-09-02: 44 in `internal/billing`, 13 in `internal/middleware`, 26 in `internal/pkg/subscription`). `DeriveStatus` and `ensureInvoicesUpTo` at 100%. No DB test infrastructure exists, so repository tests remain deliberately out of scope. | `B-Edge-Monetization-Implementation-Spec-v1.md` |
| `B-Edge-Decision-Register-v1.md` | **New, Sep 2, 2026. The single list of what this product is waiting on.** 8 resolved decisions with the file each landed in, and 11 open ones with the column that actually matters: who can settle each. Two need only an hour of engineering (D3 discount precedence and D5 review attribution both have defensible competitor-practice defaults). **D7 was closed 2026-09-02 and did not confirm the boundary - it moved it**: Stripe is still unavailable in Lebanon, but the exclusion had been justified by "Lebanon has no card rails", which is false. Tap Payments serves Lebanon and publishes both card-on-file tokenization and a Stripe-Connect-shaped Marketplace API with split payouts and sub-merchant KYC. What is NOT established is whether the marketplace product is available for Lebanon, whether payouts can settle into Lebanese banks under capital controls, or whether customers would use cards at all (60-70% of Lebanese e-commerce is still cash on delivery). That became D20, **answered 2026-09-03 and it split the excluded scope in half.** Tap is out - their support docs say *"we currently do not accept any new merchants from Egypt, Jordan, or Lebanon"* and they pay out only to GCC-domiciled businesses. But **Areeba is Beirut-based, onboards Lebanese merchants, and offers tokenization and recurring charges** - so card-on-file and automated cancellation-fee capture were **never actually blocked by the market**, and had been excluded since Aug 30 on a premise that was false. Only artist payouts appeared to remain — until checking the codebase before drafting that email dissolved the question. **B-Edge holds no money**: zero payout/ledger/settlement tables, zero commission or take-rate concepts, deposits move customer-to-artist directly over OMT/Whish, and revenue is subscriptions only. A split payout has nothing to split, and a stored card has no merchant to charge against without deciding whether B-Edge becomes a money-holding platform. **D21 is therefore a business-model question, not a vendor one**, and it also narrows what D20 appeared to unblock. Caveats that survive: settlement under capital controls is a banking question and the likelier blocker, 60-70% of Lebanese e-commerce is still cash on delivery, and the OMT/Whish flow stays either way. Three need the founder and nothing else: **D4** the service catalogue, which has the longest lead time in the whole plan and is not engineering work; **D6** Arabic priority, which D4 forces early; **D9** demand evidence for resources/split bookings. Absorbs the four founder questions stranded in the monetization spec §11, the sharpest being **D19 — who confirms payments daily**: there is exactly one admin account, and the entire revenue model assumes a human reliably checks OMT/Whish and clicks Confirm. Written because a decision that exists only in a chat transcript is not a decision the project has. |
| `B-Edge-D3-D5-Proposals-v1.md` | **New, Sep 3, 2026.** Turns the two "engineering can propose" decisions into things to accept or reject. **D3 (discounts)**: one code per booking and fixed-before-percentage, both matching Fresha — the latter noted as counter-intuitive since it gives the customer *less* than the reverse order, so it needs a named test before someone "fixes" it. Discount comes off the **balance, never the deposit**, because the deposit is a commitment device reconciled by hand over OMT/Whish by a single admin, and a per-promo deposit multiplies manual errors while weakening the no-show guard. Surcharge before discount, code released unless the customer cancels late, subscription invoices explicitly out of scope. **D5 (reviews)**: mostly already settled by the feasibility assessment §2.2 — the only open question was the primary-stylist tie-break, proposed as highest `final_price` with duration as tiebreak, written as a tested pure function even though it is unreachable until split bookings exist, because that is the moment it would otherwise be lost. |
| `B-Edge-Interval-Algebra-Decision-v1.md` | **New, Sep 3, 2026. Sprint 5 — the architectural gate, done.** A booking now occupies a typed `Occupancy` set (`internal/booking/occupancy.go`, 100% coverage) instead of a flat `[]TimeRange`. **No behaviour changed**, which is the point: 10 golden-output tests pin every slot boundary and pass unmodified either side of the refactor. Intervals are half-open `[start,end)`, matching the GIST constraint's `tstzrange(...,'[)')` so application and database agree by construction rather than coincidence. Four kinds declared; two (`buffer`, `processing_gap`) are produced by nothing yet, and `processing_gap` is the one that does not block the artist — the reason the set had to be typed *before* split bookings rather than after. **Two findings**: the plan's T5.6 ("route conflict detection through Occupancy") does not exist — booking writes are guarded by the Postgres GIST constraint, not application code, so it is closed as not-applicable; and `GetArtistBookingsForDate` has no store filter, so a cross-store booking is in *both* queries and `TravelIntervals` deliberately excludes its own span. Also settles the Sprint 13 database shape: envelope plus `booking_intervals` child table with its own constraint. |
| *(migration 033 — service buffer)* | **Sep 3, 2026. Sprint 6, built on Sprint 5.** Cleanup time after an appointment, configurable per service (0–120 min, default 0). The design decision is in the migration header: the exclusion constraint could **not** compute the buffer inline, because `timestamptz + interval` is only STABLE (it depends on session TimeZone for DST) and index expressions must be IMMUTABLE — `make_interval` and a GENERATED column were both tried against the real database first. So `bookings.blocked_until` is a stored column and the GIST constraint ranges over it, which keeps cleanup enforced by the **database** rather than an application rule a race could walk through. The redundancy is what makes the buffer **releasable**: finishing early sets `blocked_until` back and the slot reopens on the next read, with no scheduler. `end_time` is unchanged and stays customer-facing — a slot always advertises the service duration, never duration+buffer, guarded by a regression test. |
| `B-Edge-Cross-Layer-Data-Flow-Audit-v1.md` | **New, Sep 5, 2026.** Angular → Go → PostgreSQL, audited by **measurement rather than reasoning** — every table in it is output from the running stack, not inference from type declarations. The organising finding is that the three layers disagree on how many states a field has: JSON has three (absent / `null` / value), Go's `*string` has two, PostgreSQL has two — so **`null` and "absent" collapse into `nil` and a `PATCH` cannot set a nullable column back to `NULL`**. The project already met this once and solved it for exactly one field; the index's own note that *"`clear_location` exists because COALESCE cannot express remove"* is that workaround. Also documents why the `NaN` corruption was structural rather than one bad endpoint: `JSON.stringify` turns numeric `NaN` into `null`, so numbers are accidentally safe — but money crosses the wire as a **string**, `"NaN"` passes through untouched, and `'NaN'::numeric(10,2)` is the one non-finite value Postgres accepts. Records what is already right (money as a decimal string end-to-end, pointer-for-nullable applied consistently, type mismatches producing 400s rather than panics, unknown fields ignored so mass assignment is impossible) specifically so it is not "simplified" away. 8 risks with mitigations; the recommended follow-up is `CHECK (price <> 'NaN'::numeric)` on every money column — and §5.2 is worth reading for how it got there, because the obvious constraint (`price = round(price,2)`) was written first and testing showed it rejects **neither** case: PostgreSQL defines `NaN = NaN` as TRUE for numeric, and the column coerces excess scale before any CHECK runs. Two corollaries: `CHECK (price >= 0)` does not catch `NaN` either, since `'NaN'::numeric >= 0` is TRUE; and scale can never be defended at the database level, so `internal/pkg/money` is not defence-in-depth for scale, it is the only defence. |
| `B-Edge-Review-Attribution-Spec-v1.md` | **New, Sep 6, 2026. Sprint 8 — dual-layer reviews, and D5 closed.** One review per visit now carries TWO independent scores: the specialist (`reviews.rating`) and the VENUE (`reviews.salon_rating`), the latter attributed to a **store** rather than a salon because Beirut Downtown and Tripoli are different rooms. Three rules are restated in the doc precisely because each is one reasonable-looking change from being undone: **never derive one score from the other** (that is the gaming vector — an owner inflates the venue by hiring one star, so `recomputeStoreRatingTx` carries the warning in its own doc comment rather than trusting this file to be found); **the two are allowed to disagree**, and both UIs render them labelled and distinct so a great stylist in a tired room reads as informative rather than broken; and **an unanswered venue question is NULL, never 0** — the aggregate filters `salon_rating IS NOT NULL`, or every specialist-only review would count as a zero and a good venue would trend to nothing as it collected reviews. The venue question is optional by design (§2.2's survey-fatigue warning). D5's only genuinely open question, the primary-stylist tie-break, resolved as **highest `final_price`, then longest duration**, written as a pure function with 7 tests even though it is unreachable until split bookings exist — the alternative is that it gets inlined as `booking.ArtistID` and is lost the day Sprint 13 lands. Verified end to end: specialist 5 + venue 3 produced `stores.rating = 3.00` and `artists.rating = 5.00` from one review. |
...
| `B-Edge-Booking-State-Machine-Matrix-v1.md` *(in `b-edge-web/project-docs/`)* | **New, Sep 1, 2026.** All 108 (status x action) cells of the booking state machine, extracted from the code at `bb776b1`. 12 statuses, 9 actions, 2 sweeps; 9 legal transitions and 99 that must be rejected. Written because three of the twelve bugs found in earlier live passes were this exact class and none were caught by a test. **Writing it found two defects before any test ran**: (1) of the two paths to `confirmed`, only `ConfirmDepositReceived` notifies the customer - `ConfirmDeposit` sends nothing, so whether a customer learns their booking is confirmed, and now whether they get the calendar link, depends on which button the artist pressed; bounded today only because WhatsApp is not live, meaning it would have surfaced as "some customers never get confirmations" the day Twilio was switched on. (2) `refund_due` is terminal - nothing transitions out, `refunded` is in the CHECK constraint but written by nothing, and no endpoint records a paid refund, so the notification centre's `refund_due` alert is unresolvable by construction; two bookings are stuck there now, one with a $30 deposit actually paid. Also raises four product decisions, the sharpest being that `ApproveBooking` refuses a past appointment but the confirm step in the same flow does not. |
...
| `CLAUDE-v6.md` | **Superseded Sep 2, 2026 by `CLAUDE.md`** (banner added in-file). Was accurate as of Aug 15 and is useful history for the design-system migration status and the routing bug it records, but predates migrations 026-032 and the share/inbox/calendar domains. Previously described as: Supersedes v5. Verified directly against code on Aug 15: schema v19, 12 domains, both frontend apps' real routes, design-system migration status (artist-dashboard complete, customer-pwa partial by deliberate design). Test coverage for admin/audit/notification closed same day; cart-persistence decision made (ship as-is, no server cart); a real backend routing bug that silently broke guest booking was found and fixed. Read this first in any new chat. |
| `CLAUDE.md` | **CURRENT context document — the one auto-loaded into every session.** Rewritten Sep 2, 2026 and verified against code that day: 32 migrations, 29 tables, 110 route registrations, 570 tests, 17 domains, 7 leaf packages. It replaced a version dated **May 2026** that described 9 tables, ~34 endpoints and "Go version: starting fresh" — wrong by an order of magnitude, and being fed into every session as context. Leads with the five things most likely to trip up a new session, the sharpest being that **the product does not work for customers at all** until `TWILIO_WHATSAPP_FROM` is set: every notification ever queued is `dead`, 61 of them, including 38 login codes. `CLAUDE-v3` … `CLAUDE-v6` and `CLAUDE (1)/(5)` now each carry a SUPERSEDED banner so none of them can be mistaken for current. |
| `B-Edge-Cross-Layer-Data-Flow-Audit-v1.md` | **New, Sep 5, 2026.** Data-type and edge-case audit across all three tiers. Answers, per layer, what happens to `null`/`nil`/`NULL`, `""`, `NaN`/`Infinity`, missing fields, and wrong types — measured against the running stack. Read §4 (the behaviour matrix) before adding any nullable column or any field a user can clear, and §5.1 before designing a "remove this value" flow. Its two open High risks are R1 (nullable columns cannot be cleared through the API) and, now fixed but structurally instructive, R3 (`NaN` reaching `NUMERIC`). |
| `B-Edge-Security-Test-Plan-v1.md` | **New, Aug 31, 2026.** Threat matrix and executable penetration-test plan for the API border, auth boundaries and transaction workflows: 4 threat categories, 21 fraud/spam/abuse vectors, and **44 numbered test cases** (`AUTH-*`, `INJ-*`, `CLIENT-*`, `EDGE-*`, `FRAUD-*`, `SPAM-*`) with procedure, expected behaviour and remediation for each, plus severity SLAs and release-gate rules. **First executed 2026-09-05** — see its §5. 42 checks passed and 4 defects were found, all fixed the same day; the two it predicted would fail (CLIENT-04, INJ-04) both did, and INJ-04's real defect was worse than the prediction. Its §3.3 caveats still hold for everything not run: AUTH-05/06 need HTTPS, AUTH-07 and SPAM-02 need Twilio stubbed, and EDGE-01 needs written authorisation. Three things make it B-Edge-specific rather than a generic checklist: (1) **there is no edge layer** — no CDN, gateway or WAF, just one Go binary — so cache poisoning, request smuggling, TLS downgrade and subdomain takeover are marked ANTICIPATORY and would produce false passes if tested today; (2) **there is no payment gateway**, so card fraud and chargeback abuse are genuinely N/A and the real financial vector is fraud against the *human* who confirms an OMT transfer (`FRAUD-06`); (3) it names four things expected to fail before testing starts, so they are not double-counted as discoveries — chiefly that **no security-header middleware exists at all** (verified by grep: no CSP, HSTS, X-Frame-Options or nosniff anywhere). Flags `AUTH-08` as the single highest-value case: the dev OTP bypass (`326321`, accepts any phone) is separated from total customer-account compromise by one environment variable. Companion to the assessment below, which covers what is already hardened. |
...
| Testing, Quality & Gap Analysis | 11 (B-Edge-Cross-Layer-Data-Flow-Audit-v1.md and B-Edge-Security-Test-Plan-v1.md added; E2E-TEST-PLAN.md lives in b-edge-web; the CLAUDE-v* history is one row covering 7 files) |
